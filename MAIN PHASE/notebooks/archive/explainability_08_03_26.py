...
def get_valid_indices(region):
    # Con il nuovo array binned da 34 indici (0-33)
    if region == 'dyad': return [16]
    elif region == 'shoulder': return [12, 20]
    elif region == 'boundary': return [4, 28] # Questi sono NNNNNN, usati per i riferimenti
    elif region == 'global': 
...
def process_matrices(dataset, avg_dir, avg_rc):
    """Converte l'attenzione tokenizzata a 67 in una simmetrica binnata a 34."""
...
def extract_top_kmers(mask, dataset, binned_att, args, top_k=100):
    ...
    for i, sample in enumerate(data_filtered):
        ...
        for pos in valid_indices:
            if pos < len(tokens):
                t = tokens[pos]
                if 'N' not in t: # Escludiamo matematicamente le N
                    val = attn_row[pos]
                    token_attn[t] += val
                    token_counts[t] += 1
...
#SENZA STD
# def plot_position_importance_symmetric(binned_att, args):
#     print("[*] Generazione Position Importance Bar Plot (Dyad-Symmetric, 33 Bins)...")
    
#     # Tronchiamo al 33esimo bin (escludendo le basi 198-201) per avere una simmetria geometrica perfetta
#     # 33 bin * 6 bp = 198 bp. Il centro assoluto (Diade) cade esattamente nel bin 17 (96-102 bp).
#     binned_att_33 = binned_att[:, :33]
    
#     # Simmetria Strutturale della Diade: mediamo la metà sinistra con la metà destra
#     binned_att_dyad = (binned_att_33 + np.flip(binned_att_33, axis=1)) / 2.0
    
#     # Calcolo media ed errore standard sul nuovo array speculare a 33 elementi
#     mean_attn_sym = np.mean(binned_att_dyad, axis=0)
#     sem_attn_sym = np.std(binned_att_dyad, axis=0) / np.sqrt(binned_att_dyad.shape[0])

#     positions = np.arange(1, 34) # Da 1 a 33
#     labels = [f'{(p-1)*6}–{p*6}' for p in positions] # Etichette precise per 6bp ciascuno
    
#     # Indici dei bin mascherati (1-indexed per combaciare con l'array 'positions')
#     # [18:36] bp -> bin 4, 5, 6 | [162:180] bp -> bin 28, 29, 30
#     masked_positions = [4, 5, 6, 28, 29, 30]
...
#     # Soglia per evidenziare l'attenzione più alta (Top 25%)
#     thresh = np.percentile(mean_attn_sym, 75)
    
#     # Applichiamo lo stile barra per barra
#     for pos, bar, val in zip(positions, bars, mean_attn_sym):
#         if pos in masked_positions:
#             # Barre mascherate: grigio con strisce oblique
#             bar.set_color('#E0E0E0')
#             bar.set_edgecolor('#999999')
#             bar.set_hatch('////')
#         elif val >= thresh: 
#             # Barre ad alta attenzione: arancione
...
#     # Aggiunta di una Legenda per spiegare i colori
...
#         Patch(facecolor='#E0E0E0', edgecolor='#999999', hatch='////', label='Masked Boundary (NNNNNN)')
#     ]
#     ax.legend(
#         handles=legend_elements, 
#         loc='lower center',           # Il punto di ancoraggio della legenda
#         bbox_to_anchor=(0.5, -0.65),   # Posizione x=0.5 (centro), y=1.05 (appena sopra il bordo superiore)
#         ncol=3,                       # 3 elementi divisi in 3 colonne = 1 singola riga!
#         frameon=False, 
#         fontsize=12
#     )
    
#     plt.tight_layout()
#     save_path = os.path.join(args.out_dir, f'position_importance_dyad_symmetric_{args.cell_type}_MASK.png')
#     # fig.set_facecolor('#FDF7F7')
#     # ax.set_facecolor('#FDF7F7')
...
#     # Calcolo media e DEVIAZIONE STANDARD sul nuovo array speculare a 33 elementi
#     mean_attn_sym = np.mean(binned_att_dyad, axis=0)
#     # MODIFICA: Calcoliamo solo la std, rimuovendo la divisione per np.sqrt(N)
#     std_attn_sym = np.std(binned_att_dyad, axis=0) 
...
#     # MODIFICA: Passiamo std_attn_sym a yerr invece di sem_attn_sym
#     ax.errorbar(positions, mean_attn_sym, yerr=std_attn_sym, fmt='none', ecolor='#333333', alpha=0.6, capsize=3)
...
def plot_position_importance_symmetric(binned_att, args):
    print("[*] Generazione Position Importance Bar Plot (Macro-Domains)...")
    
    binned_att_33 = binned_att[:, :33]
    binned_att_dyad = (binned_att_33 + np.flip(binned_att_33, axis=1)) / 2.0

    
    # Solo media, nessuna deviazione standard per massima pulizia visiva
    mean_attn_sym = np.mean(binned_att_dyad, axis=0)

    # --- ASSE X IN BASE PAIRS ---
    positions = np.arange(1, 34)
    x_coords = (positions - 17) * 6  # La diade (bin 17) diventa 0 bp
    
    masked_positions = [4, 5, 6, 28, 29, 30]
    masked_x = [(p - 17) * 6 for p in masked_positions]

    # Figura leggermente più larga per accomodare bene le etichette di testo
    fig, ax = plt.subplots(figsize=(14, 6)) 
    
    bars = ax.bar(x_coords, mean_attn_sym, width=5.2, color='#006B7D', edgecolor='#004D5C', alpha=0.9, zorder=3)
    ...
    # Posizioni fisiche esatte: SHL * 10.4 bp
    hinges = [
        (15.6, 'H3-H4 Core\n(SHL $\pm$1.5)'),
    ...
    ax.set_ylim(0, y_max * 1.30) # Aggiungiamo un 30% di spazio bianco in alto per le etichette
    text_y = y_max * 1.08        # Posizione y dove ancorare il testo
    
    # Stile del box attorno al testo per renderlo leggibile e professionale
    bbox_props = dict(boxstyle="round,pad=0.3", fc="white", ec="none", alpha=0.9)
    
    for pos, label in hinges:
        # Linee tratteggiate a Destra e Sinistra
    ...
    # Aggiungiamo la Diade (SHL 0) al centro con il suo testo
    ...
    # Setup degli assi
    ax.set_xticks(x_coords[::2])
    ax.set_xticklabels([f"{x:+d}" if x != 0 else "0" for x in x_coords[::2]], fontsize=11)
    ...
    plt.tight_layout()
    save_path = os.path.join(args.out_dir, f'position_importance_macro_domains_{args.cell_type}_MASK.png')
...
def plot_biophysical_profiles(dataset_all, binned_att, preds_int, val_labels, args):
    ...
    fig, axes = plt.subplots(3, 2, figsize=(23,21))
    for i, metric in enumerate(metrics):
        ax = axes.flatten()[i]
        
        sns.violinplot(data=df_compare, x='Group', y=metric, order=order_plot, palette=palette, ax=ax, inner='quart', cut=0, saturation=1)
...
def plot_attention_driven_physics(dataset_all, binned_att, preds_int, val_labels, args):
    ...
    # Indici del CORE (escludiamo le N e i primissimi/ultimissimi bin)
    # I bin NNNNNN sono [3, 4, 5] e [27, 28, 29]
...
def plot_nucleotide_composition(dataset_all, binned_att, preds_int, val_labels, args):
    ...
    plt.tight_layout()
# --- FIX COLORE SFONDO ---
    ax = plt.gca()               # Recuperiamo l'asse ("il dentro") appena disegnato da Seaborn
    # ax.set_facecolor('#FDF7F7')  # Coloriamo lo sfondo interno
    # fig.set_facecolor('#FDF7F7') # Coloriamo lo sfondo esterno
...
def load_data_and_matrices(args):
    ...
    for fold in range(5):
        data = torch.load(os.path.join(args.results_dir, f"matrices_results_fold{fold}_{mat_type}_MASK_08_02_2026.pt"), map_location='cpu')
...
#     print(f"[*] Caricamento dataset: {args.cell_type.upper()}")
#     ds_map = {'lympho': 'Lymphoblastoid_99_8_percentile.pkl', 'act': 'CD4T_h19_Act_tot_99_8_percentile.pkl', 'rest': 'CD4T_h19_Rest_tot_99_8_percentile.pkl'}
#     pred_map = {'lympho': 'preds_lymphoblastoid_model_sum_folds_16_02_26_MASK.pkl', 'act': 'preds_act_model_sum_folds_16_02_26_MASK.pkl', 'rest': 'preds_rest_model_sum_folds_16_02_26_MASK.pkl'}
#     mat_type = 'lymp' if args.cell_type == 'lympho' else args.cell_type
...
#     print("[!] ATTENZIONE: Caricamento SOLO FOLD 0 per test rapido!")
    
#     # Carichiamo direttamente il file del Fold 0
#     file_path = os.path.join(args.results_dir, f"matrices_results_fold0_{mat_type}_MASK_08_02_2026.pt")
#     data = torch.load(file_path, map_location='cpu')
    
#     # Invece di fare somme e medie, prendiamo direttamente i tensori del fold 0
#     avg_dir = torch.stack(data['matrices_dir'])
#     avg_rc = torch.stack(data['matrices_rc'])
        
#     preds_int = (pd.read_pickle(os.path.join(args.results_dir, pred_map[args.cell_type])) > 0.5).astype(int)

#     # Ora avg_dir non è più None, è un Tensor di PyTorch e la funzione .numpy() funzionerà!
#     binned_att = process_matrices(dataset, avg_dir, avg_rc)
...
def main():
    ...
    dataset_all, binned_att_all, preds_int = load_data_and_matrices(args)
    ...
    if args.plot_positions: plot_position_importance_symmetric(binned_att_filtered, args) 
