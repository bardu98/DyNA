...
def plot_top_kmers(dataset, avg_dir, avg_rc, args):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f'top{args.top_k}_kmers_{args.cell_type}_{args.filter}.png'))
    plt.close()
    print("[+] Plot salvato.")
...
def plot_position_importance(avg_dir, avg_rc, args):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f'position_importance_{args.cell_type}.png'))
    plt.close()
    print("[+] Plot salvato.")
...
def plot_biophysical_profiles(dataset_all, avg_dir_all, avg_rc_all, preds_int, val_labels, args):
    ...
    palette = {'Nucleosome Anchors\n(True Positives)': '#E08E00', 'Global Background': '#A9A9A9', 'Nucleosome Exclusion\n(True Negatives)':'#008C9E'}
    ...
    for i, metric in enumerate(metrics):
        ...
        def annot_stat(x1, x2, y, h, p):
            ...
            ax.text((x1+x2)*.5, y+h, "p < 0.001" if p < 0.001 else f"p = {p:.2e}", ha='center', va='bottom', color='k', fontsize=13, fontweight='bold')
    ...
    plt.suptitle(f"Biophysical Profiles in {args.cell_type.upper()}: True Positives vs True Negatives\n {args.region.capitalize()} Region Analysis", fontsize=18, y=1.02, fontweight='bold')
    plt.tight_layout()
    plt.subplots_adjust(top=0.93, hspace=0.35, wspace=0.25)
    plt.savefig(os.path.join(args.out_dir, f'biophysical_profiles_{args.region}_{args.cell_type}.png'), bbox_inches='tight')
    plt.close()
    print("[+] Plot salvato.")
...
def plot_motif_enrichment(dataset_all, avg_dir_all, avg_rc_all, preds_int, val_labels, args):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f'motif_enrichment_{args.cell_type}.png'), bbox_inches='tight')
    plt.close()
    print("[+] Plot salvato.")
...
def load_data_and_matrices(args):
    ...
    pred_map = {'lympho': 'preds_lymphoblastoid_model_sum_folds_16_02_26.pkl', 'act': 'preds_act_model_sum_folds_16_02_26.pkl', 'rest': 'preds_rest_model_sum_folds_16_02_26.pkl'}
    mat_type = 'lymp' if args.cell_type == 'lympho' else args.cell_type

    with open(os.path.join(args.data_dir, ds_map[args.cell_type]), 'rb') as f: dataset = pickle.load(f)
    for d in dataset: d['sequence_rev'] = str(Seq(d['sequence']).reverse_complement())
...
def main():
    ...
    parser.add_argument("--region", type=str, choices=['dyad', 'boundary', 'global'], default='global')
    ...
    if args.all_plots:
        args.plot_kmers = args.plot_positions = args.plot_physics = args.plot_motifs = True
    ...
    if args.plot_positions: plot_position_importance(avg_dir_filtered, avg_rc_filtered, args)
