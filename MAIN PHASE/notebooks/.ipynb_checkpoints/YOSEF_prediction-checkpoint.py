...
def main():
    parser = argparse.ArgumentParser(description="Script di Inferenza e Estrazione Matrici (Single Model)")
    ...
    parser.add_argument("--model_path", type=str, required=True, 
                        help="Percorso del file .pt contenente i pesi del Best Model")
    ...
    file_map = {
        ...
        'rest': '../../INFERENCE/NuPoSe_Yosef_Data/RestingNuclesomal_TOT.pkl'
    }
    ...
    print(f"\n[*] Avvio inferenza con il singolo Best Model...")
    
    # 4. Inizializzazione e Caricamento Singolo Modello
    ...
    print(f"\n   -> Metriche Best Model: MCC={metrics['MCC']:.3f}, AUC={metrics['AUC']:.3f}")
    
    # 6. Salvataggio Predizioni
    nome_file_preds = f"../results/preds_{full_name}_best_model_YOSEF.pkl"
    with open(nome_file_preds, 'wb') as file:
        pickle.dump(preds, file)
    
    # 7. Salvataggio Matrici di Attenzione
    print("[*] Estrazione e salvataggio matrici di attenzione...")
    ...
    nome_file_matrici = f"../results/matrices_results_{full_name}_best_model_YOSEF.pt"
    torch.save(checkpoint_data, nome_file_matrici)

    print(f"\n✅ Operazione completata!")
    print(f"Predizioni salvate in: {nome_file_preds}")
    print(f"Matrici salvate in: {nome_file_matrici}")
